...
const api = {
  ...
  window: {
    ...
    openRoute: (routeOrOptions) =>
      ipcRenderer.send("window:openRoute", routeOrOptions),
  ...
  secureStore: {
    ...
    setToken: (service, token) =>
      ipcRenderer.invoke("secureStore:setToken", { service, token }),
    deleteToken: (service) =>
      ipcRenderer.invoke("secureStore:deleteToken", service),
  },
  ...
  printer: {
    ...
    printPDF: (filePath, options) =>
      ipcRenderer.invoke("printer:printPDF", { filePath, options }),
  },
  ...
  ipc: {
    invoke: (channel, data) => {
      ...
      if (!allowed.has(channel))
        throw new Error(`IPC invoke no permitido: ${channel}`);
    ...
    send: (channel, data) => {
      ...
      if (!allowed.has(channel))
        throw new Error(`IPC send no permitido: ${channel}`);
      return ipcRenderer.send(channel, data);
    },
    on: (channel, cb) => {
      const allowed = new Set(["window:maximized", "window:unmaximized"]);
      if (!allowed.has(channel))
        throw new Error(`IPC on no permitido: ${channel}`);
    ...
    once: (channel, cb) => {
      const allowed = new Set(["window:maximized", "window:unmaximized"]);
      if (!allowed.has(channel))
        throw new Error(`IPC once no permitido: ${channel}`);
      const listener = (_event, ...args) => cb(...args);
